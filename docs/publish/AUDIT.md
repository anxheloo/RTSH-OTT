...
- ~~**POLISH: Icon variants**~~ — **RESOLVED both halves.** iOS uses the SDK 54+ Icon Composer bundle `./assets/AppIcon.icon` ([app.config.ts:83](../../app.config.ts#L83)), which carries the light/dark/tinted variants; Android's `adaptiveIcon` now includes a `monochromeImage` layer for the Android 13+ themed icon. **Corrected 2026-09-22:** `app.config.ts` sets no `monochromeImage` (the file existed but was never wired), and the old-brand monochrome PNG was deleted with the "rtsh" rebrand — Android 13+ themed icons currently fall back to the full-colour adaptive icon. Re-open if themed-icon support is wanted: it needs a single-colour "rtsh" silhouette on transparent.
