...
- **Headers at the API layer only.** The backend picks the stream manifest/ABR ladder at the `GET /channels/{id}` call (`PlaybackDecisionDTO.streams`), where the headers are present. CDN segment/key requests carry no app headers by design (see gaps).
