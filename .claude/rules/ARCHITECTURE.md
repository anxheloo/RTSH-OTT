...
1. **Native splash** (`expo-splash-screen` config in `app.config.ts`) — shows the logo from frame zero, brand-black, and **stays up for the whole boot** (it's not hidden until React is ready). Both platforms show the lowercase "rtsh" mark (`assets/images/splash-icon.png`, 1440×943, rendered from the same SVG as the header's `RtshWordmark`) — iOS at `imageWidth: 200`, Android at `imageWidth: 160`. The "sh" is knocked out (transparent), so on the black splash it reads black inside the red disc — intended, not a missing fill. Android 12+ constrains the splash icon to a ~192dp circle: at 160dp wide (~105dp tall) every opaque pixel sits inside it, since the circle only clips the empty corners of the bounding box.
2. **Router** — `_layout.tsx` is split into **`RootLayout`** (providers only: `QueryClientProvider` → `KeyboardProvider`) and **`RootLayoutNav`** (all hooks + the `Stack`). The one-time wiring (`setupAuthRefresh`, `setupFocusManager`, `initI18n`) runs at **module scope** in `_layout.tsx` (not in render). `RootLayoutNav` calls the boot hooks directly — `useFonts`, `useCheckToken`, `useNetworkMonitor`, `useSystemTheme`, `useOTA` — and calls `SplashScreen.hideAsync()` once **all three gates resolve** (`fontsLoaded && tokenChecked && updateChecked`, the last from `useOTA` — see the OTA bullet below). `useCheckToken`'s keychain read is **async** (`expo-secure-store`), so `RootLayoutNav` **returns `null` until `fontsLoaded && tokenChecked`** (the brand-black root window / native splash holds) — the `Stack` never renders before `isAuthenticated` is known. This makes the `(auth)`/`(app)` guards key on the correct **store** `isAuthenticated` with no flash on cold boot **or JS/Metro reload** (the native splash only covers a cold launch; on a reload there is no native splash, so the `return null` gate is what prevents the login flash). Guards key on `isAuthenticated`, never the in-memory access token (null on cold boot).

Boot gates: `useCheckToken` (keychain check) + `useFonts` (Inter) + `useOTA` (published update, capped at 5s). `SplashScreen.preventAutoHideAsync()` is called at module load in `_layout.tsx`. Native rebuild required when the splash config changes (config-plugin output).
...
- **Native splash only (2026-06-17, supersedes the 2026-06-12 three-phase `BrandedSplash` handoff).** The native logo splash already covers the entire boot window (offline-first boot is near-instant), so the JS progress bar added a phase + a handoff seam for no real benefit. Removing `BrandedSplash` deletes the only place the native→JS swap could flicker. The native splash and the `BrandHeader` both use the lowercase "rtsh" mark (rebrand 2026-09-22, replacing the stacked-squares "RTSH" lockup), so boot hands off to the first screen with no brand change.
- **Hide on fonts + keychain check.** Both essentially instant. No network blocking — `useCheckToken` is a keychain read only.
- **OTA gates the splash for at most 5 seconds, then never again (reworked 2026-09-22 — superseded "OTA does NOT gate splash").** `useOTA` applies a published update **silently**: online, it checks + downloads while the native splash is still up and, if the download lands within `UPDATE_WAIT_MS` (5000), `reloadAsync`s into it behind a brand-black reload screen, so the user only ever sees the new JS. Offline (`NetInfo`), slower than 5s, or any error → the current bundle boots immediately; a download already under way finishes and applies on the **next cold start**, never mid-session. There is **no "Update available" prompt any more** — it was dropped (with its `update.ota_*` strings) because a prompt most users decline leaves them on old JS, and because the store-update notice below needs updates to land without a tap. The JS calls queue behind expo-updates' own `ON_LOAD` startup check (a serial native state-machine queue, read in `EnabledAppController.swift`), so a download the native side already started is reused, not repeated. **Trade-off:** a bad update now reaches every user on their next launch with nobody able to decline it — so production updates go out with `--rollout-percentage` first and are pulled with `eas update:revert-update-rollout` (runbook: CLAUDE.md → Releasing). Locked by `hooks/__tests__/useOTA.test.ts`.
- **Store-update notice — for what an OTA cannot ship (2026-09-22).** `constants/appUpdate.ts → STORE_UPDATE_MODE` (`'off' | 'notice' | 'block'`) tells users of **this** runtime that a newer store version exists. `'notice'` renders `StoreUpdateBanner` — a strip under `BrandHeader` that cannot be closed and opens `openStoreListing()` (the App Store / Play app, falling back to the web listing where no store app exists — iOS simulator, Android TV without Play) — and `useBrandHeaderHeight` adds `STORE_UPDATE_BANNER_HEIGHT` in the same condition, so every tab screen, radio and login already clear it with no per-screen change (device-verified on the iOS simulator: login, Home, Guide). `'block'` keeps the existing blocking `forceUpdate` modal up via `useStoreUpdateBlock` (a store subscription re-raises it if another modal takes the single slot). **The switch is only ever turned on in an OTA published to an OLDER runtime, from that release's git tag**; `main` stays `'off'`, enforced by `constants/__tests__/appUpdate.test.ts`, because a store build made from `main` would otherwise show the notice to users already on the newest version. It stays up until the user installs the new version, whose embedded bundle carries `'off'` — no version comparison needed. **Only reaches builds that contain this code:** runtime `1.0.1` once its OTA lands, and every build from `1.0.2`. Runtime `1.0` (Android builds 16/17, iOS 1.0(1)) predates it and needs its one-off OTA from its own build commit.
...
- ~~No cellular-data gate UI~~ — **built 2026-07-31.** `useCellularGate()` mounts at the top of both player routes (`channel/[id]`, `radio/[id]`) and returns `{ pending }`, which those routes gate playback on: the channel screen holds the player skeleton until playback has first started (`holdForCellular || mediaPending || adPending`, so on entry `LivePlayer` never mounts and no stream is fetched); once playback has started, a later `pending` only **pauses** the mounted player (`paused={… || cellular.pending}`) instead of unmounting it (2026-09-21) — tearing a player down and rebuilding it raced expo-video's Android playback service (REACT-NATIVE-RTSH-OTT-B) and dropped the viewer out of fullscreen, and the radio screen skips its `setRadioChannel` selection (`RadioAudioHost` is store-driven, so nothing reaches the speakers). `pending` is **derived, not stored** — exactly `connectionType === cellular && !cellularPlaybackAllowed && !cellularAcknowledged`. Continue sets the session-only `cellularAcknowledged`, which clears `pending` in the same render and releases the player; Cancel `router.back()`s. Because `pending` is reactive rather than mount-once, a mid-session Wi-Fi → cellular switch re-asks and re-holds playback — the behavior `createNetworkSlice` always documented as the intent but the old mount-only effect never delivered. The effect also clears its own modal on unmount: a route-owned `confirmation` must never outlive its route. Locked by `hooks/__tests__/useCellularGate.test.tsx`.

- **Both player routes are plain card pushes, NOT a native presentation — load-bearing, do not "restore" it.** `ModalWrapper` lives at the app root, so its RN `<Modal>` presents from the root view controller. `fullScreenModal` made the player a _second_ natively-presented VC from that same controller, and on iOS the two race: ANY global modal opened while the player is on screen (the cellular gate, `noInternet` on a mid-stream drop, `apiError`) flashed ~1s, was orphaned when the presentation settled, and — since nothing cleared `currentModal` — stranded the store at `'confirmation'`: an invisible full-screen modal window swallowing every touch app-wide once the user went back to the tabs. The card push keeps the player in the root controller's own stack, so global modals present cleanly over it. **Device-verified 2026-09-22** (iPhone 12 Pro Max, iOS 26.4, local Release preview build, live backend): automatic PiP on swipe-home plays video + audio and returns to the app on tap, with no background-playback service enabled; `noInternet` and the cellular gate both present in fullscreen landscape without crashing (REACT-NATIVE-RTSH-OTT-12), and OK resumes the paused player.

  **RETESTED 2026-08-07 — react-native-screens 4.26 / iOS 26.1, live backend: STILL BROKEN.** This is not stale caution carried forward from an old SDK. `channel/[id]` was temporarily switched to `formSheet` (`detents: [1]`) with a timer raising `noInternet` 3s after mount. The **sheet itself rendered correctly** — inset, grabber, rounded corners, stream playing — but the modal **never appeared**, and returning via the back button **froze the app to touch**. Re-probe with that exact recipe at the next SDK upgrade rather than assuming it either persists or is fixed. Separately, the probe proved `formSheet` (and `fitToContents`) work fine on iOS at 4.26 — the SDK-54-era belief that iOS formSheet is broken is version-specific and does **not** apply here, so the four `(modals)/` sheets need no platform conditional.

  **Presentation options live in `utils/navigation.ts`, not inline:** `getPlayerScreenOptions()` (card push, `slide_from_bottom`, `gestureEnabled: true` + `gestureDirection: 'vertical'`) and `getModalScreenOptions()` (formSheet). The vertical gesture — added 2026-08-07, device-verified — restores swipe-down-to-dismiss on **both** `channel/[id]` and `radio/[id]` without a native VC; `fullScreenGestureEnabled` is deliberately left off so the gesture starts at the screen edge and doesn't fight the EPG `FlashList` or the scrubber. `radio/[id]` had **no `Stack.Screen` entry at all** until then, so it slid in from the right like a settings page while `channel/[id]` slid up — the two player screens now match. **The accepted cost is rounded top corners**, which are a native-sheet affordance and unreachable on a card push; lifting that means moving `ModalWrapper` off RN `<Modal>` onto route-based `transparentModal` screens (not scoped).
...
- **Single engine above the router.** `RadioAudioHost` (`components/Media/RadioAudioHost.tsx`) is mounted once in `(app)/_layout.tsx`, wrapping the `Stack` and `RadioMiniPlayer`. It owns the only `expo-audio` player and renders no UI of its own — it wraps the router only to share that player through context (`useRadioAudioPlayer`, added 2026-09-22), so a screen can read status and seek without owning the engine.
...
- **Routes + mini-player never touch the source or play state.** `radio/[id].tsx` selects a station via `setRadioChannel(...)`; the transport + mini-player flip `radioIsPlaying`. All audio is a downstream effect of the store. `clearRadio()` (mini-player close) pauses + tears down. The one exception is **seeking a recording**: `RadioSeekBar` reads `useAudioPlayerStatus` and calls `seekTo` on the context player directly — position is not store state (it would churn the store twice a second for one screen).
...
- ~~**Live radio offered a scrub bar on the lock screen.**~~ **Fixed 2026-08-14.** `setActiveForLockScreen` was called with no `options`, and expo-audio gates the scrubber on exactly that argument (`changePlaybackPositionCommand.isEnabled = !isLiveStream`, `MediaController.swift`) — so the lock screen advertised a duration and an **enabled seek** for a stream that has neither. The host now passes `{ isLiveStream: radioProgramId == null }` — `true` for live, `false` for a catch-up recording (added 2026-09-22), which is exactly what gives a recording a working lock-screen scrubber. `showSeekForward`/`showSeekBackward` stay at their `false` default for the same reason.
- ~~**No radio-EPG source.**~~ **Resolved, with a day strip (2026-07-27).** `radio/[id].tsx` reuses the SAME per-channel EPG endpoint/hook AND day-strip mechanism as the TV channel screen (`useChannelEpgQuery`, `GET /channels/{id}/epg?date=YYYY-MM-DD`, the 7-back/today/7-forward `DayStrip` + `CatchupBanner`) — a radio station is a `Channel` with `type: 'RADIO'` in the same id-space, so no new endpoint or param was needed. ~~Browsing is read-only~~ — **superseded 2026-09-22: radio has catch-up.** The backend flags every radio programme `hasCatchup: true` and serves `GET /channels/{id}/epg/{programId}` for radio exactly as for TV (verified against the live API: guests get `LOGIN_REQUIRED`), so row states now match `channel/[id]`: TODAY's airing row is `now` (toggles live, or returns to live from a recording), every finished programme is `recorded`, only not-yet-started ones are `scheduled`. A recorded tap `fetchQuery`s the shared `channelPlaybackQueryOptions` cache entry, shows the server's `noticeMessage` in a `notify` modal on a non-`ALLOWED` decision, and otherwise calls `setRadioChannel({ …, programId })`. The recording plays through the **same** `RadioAudioHost` engine — `PlayerSlice.radioProgramId` (`null` = live) is the only new state — so it survives navigation, docks in the mini-player (titled with the programme), opens a `RECORDED` watch segment, and gets a lock-screen scrubber. Guests hit `promptSignIn()` before any fetch (`GUEST_CATCHUP_ALLOWED`); a last-tap-wins counter drops a slow decision the user has since overridden or swapped station away from; and re-entering the screen from the mini-player does not yank a recording back to live. The list auto-centers on whatever is on the engine (the recording, else the airing row).

  **Seek bar (2026-09-22).** While a recording is loaded, `radio/[id]` mounts `RadioSeekBar` (`components/Media/`) under the transport: elapsed / total + a track with the same behaviour as the TV scrubber in `PlayerControls` — tap to jump, drag to scrub, and the bar holds the release point until playback reaches it (or clears it if the seek rejects). The gesture runs on the JS thread (`runOnJS(true)`), since audio has no frame to keep in sync with. Screen readers get `accessibilityRole="adjustable"` with ±15s increment/decrement. Live radio never shows it.

  **Device-verified 2026-09-22, live backend, demo account:** Android emulator (Pixel 6 API 33) — today's and yesterday's recordings play from their start, lock-screen seek on for a recording and off for live, pause/resume keeps position, re-entering from the mini-player keeps the recording, LIVE row and next-station return to live. iOS simulator (iPhone 17 Pro, iOS 26.5) — a guest tapping a recording gets `signInRequired` while live keeps playing; as a member the seek bar shows the right duration (26:59 for a 27-min slot), tap-to-seek and drag-to-scrub both land and keep playing, and the bar disappears on return to live.

  The in-app seek bar was also run on the Android emulator the same day: tap-to-seek landed at 13:41 (the OS media session reported the same 821 s) and a drag landed at 22:41, both continuing to play. On Android the total can read a few seconds (`0:20`) for the first moment of a recording while ExoPlayer loads the playlist, then settles to the real duration.

  **Play at the end restarts (2026-09-22).** Both native engines park a finished recording AT its end, where `play()` is a no-op — so on TV the button stayed on "play" and did nothing, and on radio the store flipped to playing while the engine stayed silent (a pause icon over silence). `isAtPlaybackEnd()` (`utils/playback.ts`, 1 s tolerance, never true without a finite duration) now gates both play paths: `PlayerControls` calls `player.replay()` before `play()` for a non-live source at its end, and `RadioAudioHost` does `seekTo(0)` then `play()` when a recording's intent turns on at its end. Device-verified on iOS (radio 3-min recording, TV 12:30 news): play after the end restarts from 0:00 and actually plays.

  **No parental gate on recorded radio — by product decision (2026-09-22),** not a gap: radio has none on live either. Catch-up still needs an account (`GUEST_CATCHUP_ALLOWED`; Android is login-only anyway).

  **Known gaps (radio catch-up):** the seek bar is **not D-pad operable** on TV (a pan gesture, like the TV video scrubber).

  **Layout + virtualization (reworked 2026-07-27).** The screen is a fixed two-pane body under a **floating frosted header** (the `BrandHeader` treatment — `position: absolute` + iOS `BlurView` / Android translucent solid — rather than the flat in-flow `TabHeader` that settings/account use; the favourites action was dropped). The top pane (now-playing core) and the day strip are both **fixed**; the schedule list is the only scroller. The top pane is sized to its own content rather than a `flex: 1` half — a forced 50/50 split squeezed `RadioPlayer` into overlapping the pane below, so its internals were also given a compact pass (art 160 / smaller transport). The schedule is a **`FlashList`**, not a `.map()` in a `ScrollView`: a `ProgramRow` is expensive to mount (reanimated layout + worklet, plus a native `BlurView` on Android), and building a whole day at once made switching dates visibly janky. Virtualizing also replaced the `onLayout` offset bookkeeping with a plain `scrollToIndex` for the auto-center — nothing to measure, nothing to invalidate on a day change.
...
- ~~**iOS store URL placeholder**~~ — **resolved 2026-09-21:** `IOS_APP_STORE_ID = '6803853740'` (the ASC Apple ID, same as `eas.json` → `ascAppId`). Builds before this carry `''`, so their iOS 426 CTA is a no-op — never raise a 426 against iOS runtime `1.0`.
...
- ~~**Every global confirmation modal was unusable on TV**~~ — **FIXED 2026-08-17, device-verified.** `ModalWrapper`'s backdrop + sheet are two nested `TouchableOpacity` wrappers around the buttons; on TV they were focusable ancestors, so the D-pad was trapped on the wrapper and **no button could be focused at all** — logout, delete-account, `apiError` retry, `noInternet` and the blocking `forceUpdate` CTA were all visible but unpressable in every direction, with Back the only escape (and Back is deliberately disabled on `forceUpdate`, so a 426 on TV would have been an unrecoverable app). This is the *same* trap `STYLE_GUIDE.md → "Nested focusables trap the D-pad"` already documented and that `RadioMiniPlayer` / `PlayerControls` were already fixed for — `ModalWrapper` was simply missed. Fix is two parts, and **the first alone is insufficient**: `focusable={isTV ? false : undefined}` on both wrappers, **plus** `hasTVPreferredFocus={isTV ? true : undefined}` on the primary button, because an RN `<Modal>` is its own window and with the wrappers unfocused nothing inside it is focused at all. **Write the guard as `isTV ? false : undefined`, never `!isTV`** — `TouchableOpacity` defaults `focusable` to false when it has no `onPress`, so the `!isTV` form *adds* Android focusability off-TV; the two earlier call sites both have an `onPress`, which is why the older form never showed the defect.

- ~~**Registration was impossible on Android TV**~~ — **FIXED 2026-08-17, device-verified.** D-pad focus dead-ended at the "Edukimi" field: `describe` reported it as the **last focusable in the tree**, and 13 `down` presses plus `right` left the screen byte-identical, stranding the terms checkbox and the submit button. Root cause is **not** clipping or the `enableCustomFocusSearchOnClippedElementsAndroid` flag (that defaults `false` in both the Kotlin and C++ sources on this RN, confirming plan 22.18-TV.b's "dead code" claim) — it is `SelectInput`, the **one** component in the app built on `@expo/ui` (`Host` + `Picker`), i.e. a native **Jetpack Compose** island embedded in the RN tree. Compose owns its own focus system and does not hand the D-pad back. **This was pre-registered as a risk** in plan 22.18-TV ("`@expo/ui` Picker D-pad focus on Android Compose") and had simply never been exercised. Fix: `SelectInput` renders `SegmentedChoice` when `isTV` and keeps the native menu off-TV — education has exactly 3 options and the same form already uses that control one field above for gender. Two paired gaps surfaced on the same path and were fixed with it: `SegmentedChoice` and `Checkbox` had **no focus ring at all**, so on TV they were navigable but invisible (both now use `tvFocusHighlight(colors.focus, focused, { scale: false })`; `SegmentedChoice` needed its option extracted into a `Segment` sub-component so each can hold its own `useTVFocus`). `Checkbox` mattered because accepting the terms is *required* to register.
  **Standing rule this establishes:** a native-interop island (`@expo/ui`, any Compose/SwiftUI view) on a TV-reachable screen needs an `isTV` branch built from plain RN. It is invisible on touch and `@expo/ui` does not claim TV support.
...
- **TV/tablet large-screen display-adjustment pass (22.18) is not done.** The current screens still run the **mobile portrait** layout on TV (the channel screen and — since 2026-08-06 — `radio/[id]` are the only ones with a TV branch) — on a 1920×1080 screen the portrait inline video (`width:100%` + `aspectRatio:16/9`) renders at the full screen height, so anything stacked below it (day-strip, EPG list on non-channel screens) sits below the fold with nothing to scroll it into view. Root-caused 2026-07-07 (confirmed via a UI-tree dump on a real TV emulator boot) as a **layout** gap, not a focus bug — the D-pad/focus foundation itself works (Home grid nav, focus rings all verified on-device).
  ...
  **Severity corrected 2026-08-17 — this entry overstated the remaining layout risk, and the two real blockers were focus bugs it explicitly ruled out.** A full remote walk of the production APK against the live backend (960×540dp, `mCurUiMode=0x24`) found the non-branched screens **render and scroll correctly**: Profile and Settings both reach their last row (delete-account / version) because a `ScrollView` follows D-pad focus; auth forms scroll; the `(modals)` sheets auto-focus their first option, hold focus against `up`, and dismiss on Back; Home's grid, the route menu, the channel player, the guide drawer and live HLS playback all work. The "stacked below the fold with nothing to scroll it into view" wording describes the **pre-drawer** channel screen and no longer generalizes. What *did* fail were the two focus defects above — so the "the focus foundation itself works" claim was true only for the surfaces that had been walked. **What remains genuinely unverified is overscan (TV-OV): an emulator renders 1:1 into a host window and has no overscan at all**, so a zero-margin layout looks perfect there and can still be clipped on a real panel — the player's back chevron and pause button sit ~30dp from the edges against a ~48/27dp guidance margin. Also still open: no screen sets `hasTVPreferredFocus` outside `ModalWrapper` and the channel drawer, so most screens mount with nothing focused; and Home's grid was seen rendering blank cells in columns 1–2 after returning from a channel, painting only once focused (reproduced 3×, self-heals on further scrolling, not attributed between TV and the software renderer).
...
- **OTA symbolication pulls the token from `eas env` at publish time (reworked 2026-07-31).** The OTA
  ...
  **On-device verification** uses the Settings → Version row, which appends `Updates.updateId`'s
  first 8 chars (`settings.tsx`). It shows the id of whichever bundle is running, so
  `RTSH TANI 1.0.0 (019fbdc9)` identifies *which JS the device is running* —
  otherwise two bundles look identical and "the update worked" is a guess.

  **Corrected 2026-08-17: the suffix is NOT empty on an embedded bundle, so its presence alone proves
  nothing.** This previously read *"It is empty on the embedded bundle and shows the id once an update
  is applied"* — false. A locally-built **production** APK displayed `RTSH TANI 1.0 (1a8e9151)` while
  `eas branch:list` showed the **production branch carrying zero updates** (only `preview` (iOS) and
  `review` have any), so no OTA could have applied: `Updates.updateId` returns the **embedded**
  update's own id. Read the suffix as "which bundle", never as "an OTA is applied"; to tell embedded
  from OTA, compare the id against `eas update:list` for that channel. `useOTA` checks once per
  **cold start** and applies silently (see → Boot / Splash gate), so a full app **force-close** is
  required to pick up a new update; backgrounding does not.
...
- ~~**The App Store Connect listing icon carries an alpha channel.**~~ **Closed 2026-09-22** with the
  "rtsh" rebrand: `appstore.png` (and `assets/images/icon.png`) were re-saved as RGB, no alpha.
  Watch for it on the next icon change — generator packs (appicon.co) ship the 1024 with an
  all-opaque alpha channel, and Apple rejects the channel's presence, not just transparency.
- The remaining gap does not block development or an internal/preview build — both are pre-submission blockers
