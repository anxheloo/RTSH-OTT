...
5. **Logout** — the mutation reads the keychain refresh token and POSTs `/auth/logout { refreshToken }` best-effort (the refresh token identifies *which session* to revoke; the access token only identifies the user). `useAppStore.logout()` stays the async single chokepoint for the local wipe: removes refresh token + PIN cache from keychain, clears store auth state; the mutation also `queryClient.clear()`s cached data.
...
- **`parentalPinSet` missing from the backend `UserDTO`** — the parental gate's fresh-device hydration depends on it; asked backend 2026-06-12, `userDtoSchema` already accepts it as optional. Until it lands, a fresh device won't know a PIN exists before the first verify.
...
- **`isPinSet` hydration.** `login()` seeds `ParentalSlice.isPinSet` from `parentalPinSet` on the auth/`getMe` payload, and `isPinSet` is itself persisted — so the gate is known before any network on a warm boot, and accurate on a fresh device. ⚠️ `parentalPinSet` is **not yet on the backend `UserDTO`** (asked 2026-06-12); the client already accepts it as optional.
