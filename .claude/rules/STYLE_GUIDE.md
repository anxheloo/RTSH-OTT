...
- **Cross-scroller focus is unreliable; use ONE list.** RN can't reliably hand the D-pad **between two scrollers** (a horizontal date strip above a vertical `ScrollView`). The fix is the same as [Lists](#lists--screen-composition): a **single vertical `FlatList`** with the other region as `ListHeaderComponent`, so focus flows within one list (it also auto-scrolls the focused row into view). See `channel/[id].tsx` → `tvGuideList`. A plain `ScrollView` list additionally needs `TVFocusZone` + an `onFocus`-scroll; the `enableCustomFocusSearchOnClippedElementsAndroid=false` MainApplication flag (`plugins/withAndroidTVFocusFix.js`) is what lets focus descend into a `ScrollView` at all (RN 0.80+ #1087 regression).
