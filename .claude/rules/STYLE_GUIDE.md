...
- **Nested focusables trap the D-pad.** A focusable wrapper around focusable children (a pressable strip containing pressable buttons — `RadioMiniPlayer`; a full-screen tap overlay over control buttons — `PlayerControls`) makes the inner controls unreachable. Drop the wrapper from the focus order on TV with **`focusable={!isTV}`** (keep it for touch on mobile).
