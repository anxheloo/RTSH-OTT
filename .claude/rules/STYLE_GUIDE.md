...
> **Scope: project-specific.** This guide names concrete RTSH-OTT components, modules, and files. For the **portable, project-agnostic principles** behind these conventions — reusable on any Expo/RN project — see [`STANDARDS.md`](./STANDARDS.md).
...
**A route that opens global modals must NOT be presented as a native modal.** `ModalWrapper` lives at the app root, so its RN `<Modal>` presents from the *root* view controller. A screen with `presentation: 'fullScreenModal' | 'modal' | 'formSheet'` is a second natively-presented VC from that same controller, and on iOS the two race — the loser is orphaned: visible for ~1s, then an invisible full-screen modal window that eats every touch app-wide (nothing clears `currentModal` on unmount, so it survives navigation). This bit `channel/[id]` for real, and **retesting on react-native-screens 4.26 / iOS 26.1 (2026-08-07) confirmed it is NOT fixed upstream** — the probe (`formSheet` + a timer raising `noInternet`) rendered the sheet fine but never showed the modal, then froze the app to touch on back. Re-probe with that recipe at the next SDK upgrade; do not assume either outcome.

`presentation: 'formSheet'` sheets (`getModalScreenOptions()`) are fine _because they never raise a global modal_ — don't add one to a sheet route.

**Modal semantics on a screen that CAN raise a global modal — use `getPlayerScreenOptions()`** (`utils/navigation.ts`): a card push with `animation: 'slide_from_bottom'` + **`gestureEnabled: true` + `gestureDirection: 'vertical'`**, which gives swipe-down-to-dismiss without a native VC (iOS-only; Android keeps back). Leave `fullScreenGestureEnabled` off — a whole-screen vertical gesture fights inner list scrolling and player scrubbers; the default edge-start is also the "swipe down from the top" users expect. What this **cannot** give you is the rounded top corners: those are a native-sheet affordance (`sheetCornerRadius` on `formSheet`; UIKit's own radius on an iOS `modal`, which is where SOLITAR's iOS corners actually come from — its `sheetCornerRadius` is dead on that branch). Lifting that limit means moving `ModalWrapper` off RN `<Modal>` onto route-based `transparentModal` screens first.
...
**Every RN `<Modal>` that can open while the player is fullscreen must declare `supportedOrientations={['portrait', 'landscape']}`.** An iPhone Modal defaults to portrait-only; presented while the app is locked to landscape it shares no orientation with the app and iOS throws `UIApplicationInvalidInterfaceOrientation` — a fatal crash that **dev builds only log** (RN returns all orientations in DEV), so it surfaces only in release. This was the top iOS crash (REACT-NATIVE-RTSH-OTT-12: the cellular gate firing on a Wi-Fi → cellular switch mid-fullscreen). `ModalWrapper`, `ParentalPinModal`, `AdOverlay` and `TVNavButton` declare it; `ModalWrapper.test.tsx` pins every global modal type.
...
- **`scrollToIndex` on a `FlashList` must wait for `onLoad`.** FlashList deliberately renders no items on its first cycle — it measures itself first — so a scroll issued from a mount effect (even deferred a frame) resolves against an unmeasured list: it lands at an estimated offset and then visibly snaps to the true position on the user's first scroll. Gate it on an `onLoad`-set flag. Make the FIRST center `animated: false` (the screen should simply open there) and later ones animated (the move should read as a change). Both EPG lists do this — see `channel/[id].tsx` → `listReady`.
- **Reanimated layout/state animations must be OFF inside a recycling list.** A `FlashList` cell is re-pointed at a different item in place; Reanimated can't distinguish that from a real state change, so a `layout={LinearTransition}` plays the *swap* as if it were a resize and a `withTiming` in a `useAnimatedStyle` eases to the new item's value — both read as a flicker while scrolling ([FlashList → Reanimated + recycling](https://shopify.github.io/flash-list/docs/guides/reanimated/)). Give the row an explicit opt-out prop the virtualized parent sets (`ProgramRow`'s `recycled`) rather than deleting the animation — an unvirtualized parent (the channel screen's `ScrollView`) has no recycling and should keep it. Same rule for `useSharedValue`: reset it on item-id change, or the previous item's value bleeds in.
...
- **Tune in one place:** `responsive/breakpoints.ts` (`GRID_COLUMNS`, `GRID_GAP`, `UI_SCALE`, `TABLET_MIN_SHORTEST_SIDE`, `CONTENT_MAX_WIDTH`). Do not scatter breakpoints. `GRID_GAP` is the grid gutter per device class (phone 8, tablet/TV 14) — deliberately raw dp, not a `scaled()` token, so tablet and TV share one visibly wider gap.
...
- **Focus ring** — an interactive primitive tracks focus with **`useTVFocus()`** (`{ focused, focusProps }`, `focusProps` = `onFocus`/`onBlur`) and applies **`tvFocusHighlight(colors.focus, focused)`** **LAST** in its `style` array (it returns `undefined` off-TV *and* when unfocused, so it never clobbers the base style). This is already wired into the shared primitives (`ReusableBtn`, `ListRow`, `ChannelCard`, `ProgramRow`, …) — reuse them; don't hand-roll a ring.
  - **Full-width elements must pass `{ scale: false }`** — `tvFocusHighlight(color, focused, { scale: false })`. The default 1.05 scale "pops" compact controls (cards, icon buttons, PIN keys) but on a full-width element (search bar, list rows) it pushes the box ~5% past the screen edge → **horizontal overflow**. Border-only there. Already applied to `SearchBar`, `ListRow`, `SheetOptionRow`, `GuideRow`, `SearchResultRow`, `StationRow`.
  - **List rows want fill, not a ring** — a thin ring on a full-width transparent row is invisible on a 10-foot screen. `ProgramRow` (and the TV route-menu rows) instead render a **solid fill (`colors.surfaceHigh`) + a bold left accent bar (`colors.focus`)** when `focused` (only ever true on TV, so no `isTV` gate needed). Prefer this for any full-width drawer/list row.
  - **The ring must contrast with the FILL it sits on, not with the page background.** `tvFocusHighlight` draws a *border on the focused control itself*, so a ring the same colour as that control's `backgroundColor` gives no focus feedback at all — and on a pointerless device, a control you can't see focus on is a control you can't operate. **Fixed at the token 2026-08-10:** `colors.focus` is now `#FFFFFF` (dark) / `#0B0B0F` (light), so plain `tvFocusHighlight(colors.focus, …)` is correct on filled *and* transparent controls, including `ReusableBtn`'s `primary` and `destructive` variants. **Until then `focus` was `#EB122F` — the same value as `primary`** — which drew a red ring on every red-filled CTA app-wide (auth submit, confirm buttons, the parental PIN confirm); `RadioPlayer` carried a one-off `colors.onPrimary` override for it, now removed. **The rule that outlives the fix:** if you introduce a fill whose colour approaches `colors.focus`, pass a contrasting token explicitly rather than assuming the default ring shows — that is why `AdOverlay` rings in white and `PlayerControls` rings in `PLAYER_COLORS.brand` over video. See `ARCHITECTURE.md → Android TV / STB → Known gaps` for the decision record.
- **Focus zones** — wrap a region in **`TVFocusZone`** (a `TVFocusGuideView`, a bare fragment off-TV): `autoFocus` redirects the D-pad to the first focusable on entry; `trapFocus{Up,Down,Left,Right}` traps focus inside a drawer/overlay. Set `autoFocus={false}` when a specific child claims initial focus via `hasTVPreferredFocus` (so the guide doesn't fight it).
  - **Initial focus** — pass `hasTVPreferredFocus` to the control that should own focus when a screen/drawer mounts (react-native-tvos prop; `ProgramRow` forwards it). The channel guide drawer defaults focus to the **now-airing** row this way.
- **Cross-scroller focus is unreliable; use ONE list.** RN can't reliably hand the D-pad **between two scrollers** (a horizontal date strip above a vertical `ScrollView`). The fix is the same as [Lists](#lists--screen-composition): a **single vertical `FlatList`** with the other region as `ListHeaderComponent`, so focus flows within one list (it also auto-scrolls the focused row into view). See `channel/[id].tsx` → `tvGuideList`. A plain `ScrollView` list additionally needs `TVFocusZone` + an `onFocus`-scroll. (Historical note: a `withAndroidTVFocusFix` config plugin used to force `enableCustomFocusSearchOnClippedElementsAndroid=false` for the RN 0.80+ #1087 regression — **deleted 2026-07-28**, since react-native-tvos 0.86 already ships that flag defaulting to `false`. The single-`FlatList` rule above is what actually carries the D-pad, and always was.)
- **Nested focusables trap the D-pad.** A focusable wrapper around focusable children (a pressable strip containing pressable buttons — `RadioMiniPlayer`; a full-screen tap overlay over control buttons — `PlayerControls`; **a backdrop + sheet wrapping the modal's buttons — `ModalWrapper`**) makes the inner controls unreachable. Drop the wrapper from the focus order on TV.
  - **Write it `focusable={isTV ? false : undefined}`, NOT `focusable={!isTV}`.** The `!isTV` form passes an explicit `true` on touch builds, and `TouchableOpacity` defaults `focusable` to **false when it has no `onPress`** — so on a tap-catching wrapper (exactly the kind this rule targets) `!isTV` *adds* Android focusability that wasn't there before. `undefined` leaves the default untouched, which is what "inert off-TV" has to mean. Caught 2026-08-17 while fixing `ModalWrapper`; the two earlier call sites both have an `onPress`, so they were unaffected and the older form survived unnoticed.
  - **Unfocusing the wrappers is necessary but NOT sufficient inside an RN `<Modal>`.** A `<Modal>` is its own window; with the wrappers out of the focus order nothing inside is focused at all and the D-pad has no entry point. Give the primary action **`hasTVPreferredFocus={isTV ? true : undefined}`**. `ReusableBtn` already forwards it via `...rest` — no prop plumbing needed. **Every global confirmation was unusable on TV until this landed** (device-verified 2026-08-17: logout, delete account, retry and the blocking force-update CTA were all visible but unpressable).
  - **When the inner control only exists to split a mobile gesture, drop the split instead.** `ProgramRow` splits its press targets on mobile (body = expand the description, play glyph = play) — but a remote has no second tap target to disambiguate, so on TV the glyph is rendered as a plain `View` (never a nested pressable), row press keeps playing, and the *secondary* action rides **focus** instead (the parent sets `expandedProgramId` from `onFocus`). Prefer re-mapping a mobile-only split onto D-pad focus over making both targets focusable: focus is unique, so a "only one open at a time" invariant falls out for free from the same state field.
- **Overlays must clear a clipped/absolute parent via `Modal`.** The header route menu escaping `BrandHeader`'s `overflow: hidden` renders in a **`Modal`** (`transparent`, `onRequestClose` closes on remote Back) so it paints above the whole screen incl. the tab bar — not an absolute `View` (which is clipped to the header). A drawer inside a full-screen container (`channel/[id]`'s guide) can stay an absolute `View`.
- **TV navigation = header, not bottom tabs.** The bottom tab bar is **hidden on TV** (`tabBarStyle: { display: 'none' }` when `isTV`; `useTabBarHeight()` returns 0) — a bottom bar is poor 10-foot UX. Sections switch from **`TVNavButton`** (a route-menu `Modal` drawer in `BrandHeader`, TV-only, right side). Player controls **don't auto-hide on TV** (no tap-to-reveal; hidden chrome can't be focused).
...
| Reanimated `layout`/`withTiming`/`useSharedValue` left on inside a recycling `FlashList` row | Opt the row out in virtualized parents (`ProgramRow`'s `recycled`) / reset shared values on item-id change — otherwise cell reuse animates as a flicker |
| `scrollToIndex` fired from a mount effect on a `FlashList` | Gate it on `onLoad` — items don't exist in the first render cycle, so it lands at an estimated offset and snaps on first scroll |
...
| `presentation: 'modal'`/`'formSheet'`/`'fullScreenModal'` on a route that raises a global modal | `getPlayerScreenOptions()` — card push + `gestureDirection: 'vertical'`. The native VC races `ModalWrapper`'s RN `<Modal>` and strands an invisible touch-eating window (re-confirmed broken on screens 4.26 / iOS 26.1, 2026-08-07) |
| A sheet route sitting flat beside pushed routes | Put it in the `(modals)/` route group; href keeps the group (`/(app)/(modals)/theme`) |
| A native-interop control (`@expo/ui` `Host`/`Picker`, any Compose/SwiftUI island) on a screen that must work on TV | Give it an `isTV` branch built from plain RN (`SelectInput` renders `SegmentedChoice` on TV). Compose owns its own focus system and does **not** hand the D-pad back to React Native, so focus enters the island and never leaves — everything below it on the screen becomes unreachable. Device-verified 2026-08-17: this made the register form's terms checkbox + submit button unreachable, i.e. **signup was impossible on Android TV**. It is invisible on touch, and `@expo/ui` does not claim TV support. |
| `<NavigationBar hidden />` (expo-navigation-bar component) | The `expo-navigation-bar` config plugin (`hidden: true`) — applied natively on every Activity start. The component resets the bar on unmount via a floating `setStyle` promise, which rejects when the Activity is already destroyed (Back / low memory with JS alive) — REACT-NATIVE-RTSH-OTT-3, 117 users on build 20. Until every build carries the plugin, `_layout.tsx` hides it with `setVisibilityAsync('hidden').catch()`. |
| `WebBrowser.openBrowserAsync(url)` at a call site | `openLink(url)` (`@/utils/openLink`). Custom Tabs launches whatever app claims the browser role, and on an Android TV box that is often a video player whose web activity is not exported — the SecurityException is thrown natively inside expo-web-browser's `BrowserProxyActivity.onCreate`, so **no JS `try`/`catch` at the call site can prevent the crash** (REACT-NATIVE-RTSH-OTT-M, a real production crash). `openLink` keeps the in-app browser off-TV and uses `Linking` on TV, where a missing browser rejects instead of crashing. |
| `aspectRatio` + a **percentage** width + `maxWidth` on the SAME node | Clamp **both** axes (`maxWidth` *and* `maxHeight`), or drop the percentage for a fixed width. Yoga derives the aspect height from the percentage-resolved width and clamps only the width, so the box silently becomes a rectangle once the parent exceeds `maxWidth / percent`. Bitten twice: `channel/[id]`'s player (2026-07-28) and `RadioPlayer` + `radio/[id]`'s skeleton (2026-08-06). **Beware `contentWidth`** — it contributes a `maxWidth` from a *different* file, so the two halves of the trap can sit in separate places and look innocent apart. |
