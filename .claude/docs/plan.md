...
- [x] **11.X.14** Delete account (2026-06-25; scope = auth · parental · `/users/me`). `DELETE /users/me` (no body — access token identifies the account) via `deleteAccount` (`services/users.ts`) + `useDeleteAccountMutation`. Profile screen: "Delete account" `ListRow` next to Logout (error color, `WarningIcon`), behind the existing `confirmation` modal (destructive button). **Local wipe runs only on 200** (`onSuccess`, not `onSettled`) — `store.logout()` + new `ParentalSlice.clearParentalConfig()` + `queryClient.clear()`; a failed delete keeps the user signed in and surfaces via the global `apiError` modal (no error-suppressing `meta`). Deletion is the **only** path that clears the device-level parental gate; logout deliberately leaves it intact (per user, 2026-06-25). i18n en+sq (`profile.delete_account` / `delete_confirm_title` / `delete_confirm_message`); mock `DELETE /users/me` → 200. Docs synced (ARCHITECTURE 5a + Parental note + update log, API.md, CLAUDE.md). **Confidence:** tsc + lint clean, en/sq parity [CERTAIN].
...
- [~] **22.18** Tablet / iPad / **TV** large-screen pass (decisions 8 + TV scope). *Foundation landed 2026-06-18:* the portable **`@/responsive`** module (device-class + orientation grid columns via shortest-side classifier, `scaled()` per-class token step) is in place and wired to the Home grid + `FONTSIZE`/`SPACING`/control tokens. **Tablet content-width pass landed 2026-06-25:** `useContentWidth(variant)` (`CONTENT_MAX_WIDTH` form 480 / content 640 / player 820 — centered cap on tablet/TV, no-op on phone) wired across auth forms, change-password, settings, account, profile, radio, the channel player (video + day-strip + EPG, conditional style preserving the single-tree rotation invariant), and Guide / Search row lists. **Remaining for this step:** on-device tuning of `GRID_COLUMNS`/`UI_SCALE`/`CONTENT_MAX_WIDTH`, and the **TV** sub-pass below. ~~Mosaic density (4/6/9)~~ — moot, mosaic cut 2026-06-11 (22.14f).
  - **22.18-TV — Android TV + STB enablement (decided 2026-06-25; verified vs SDK 56 TV guide; resynced 2026-06-29).** Scope decisions: test via local prebuild + Android TV emulator (and EAS `*_tv` + Orbit); **full-polish** focus/D-pad nav across ALL screens; **keep the bottom Native Tabs, made focusable** (no separate TV nav layout); **STB self-update deferred** to a later sub-step.
    - **Resync 2026-06-29 — what changed since the plan, and how it lands on TV:** (1) **Orientation is already TV-guarded** — the `useOrientation.ts` lock helpers early-return on `Platform.isTV`, and fullscreen is now **button-driven only** (no sensor auto-rotation). So no TV orientation work remains; that earlier risk is closed. (2) **Realtime (STOMP, `@stomp/stompjs` + `fastestsmallesttextencoderdecoder` polyfill)** and **analytics (`src/analytics`)** are **pure JS, no native modules → TV-safe**; presence / watch-time / mid-roll / geo on the channel screen run on TV unchanged. (3) **Merged ads** (`useAdsQuery` → preroll + `AdOverlay` + mid-roll scheduler) — the overlay's skip/countdown chrome must be **focusable** on TV (folded into Phase C). (4) `device.ts` is **fully wired** already (`ANDROID_TV`/`STB_ANDROID` types, `DeviceClass` `TV`/`STB`, `DevicePlatform` `AndroidTV`/`AndroidSTB`) — no change. (5) `expo-glass-effect` is a dep but **unused in `src/`** → no Android-TV glass-fallback concern. (6) Project now has a **scripts convention** (`prebuild:{dev,preview,prod}`, `android:{dev,preview,prod}`, `eas:android:{...}`) — TV scripts must follow it (Phase A). tsc baseline clean.
    - **Phase A — build & boot:** add `@react-native-tvos/config-tv` (dev dep) to `app.config.ts` plugins (self-gates on `EXPO_TV`/`isTV` — safe to list unconditionally, no-ops on mobile prebuild); swap `package.json` core to `"react-native": "npm:react-native-tvos@0.85-stable"` (**npm package alias** = install the tvos fork under the name `react-native`; strict superset so mobile/tablet builds are byte-identical, TV-ness only activates at prebuild with `EXPO_TV=1`; required because every dep does `import from 'react-native'` and a project holds only one RN core — `0.85-stable` → `0.85.3-2`, matches our `0.85.3`); add EAS profiles `development_tv` / `preview_tv` (`env: { EXPO_TV: "1" }`) + `preview_stb` (`EXPO_TV:1` + `APP_PLATFORM: androidstb`); add npm scripts `prebuild:tv` / `android:tv` / `eas:android:tv` (+ `:stb`) matching the existing convention, each exporting `EXPO_TV=1`. Bring-up: `EXPO_TV=1 npx expo prebuild --clean` → `expo run:android` on a TV emulator. **Local native dirs are gitignored CNG output** — prebuild regenerates them; toggling `EXPO_TV` swaps the local project between mobile/TV (can't hold both at once). Verify `Platform.isTV` → `getDeviceClass()='tv'` (4/4 grid, 1.3× scale, content caps) + backend `deviceClass` `TV`/`STB`.
    - **Phase B — launcher presence:** TV banner (320×180) + leanback launcher intent (config-tv emits most) + TV icon/splash; confirm in the Android TV launcher.
    - **Phase C — focus & D-pad nav (the bulk):** reusable focus-ring convention on `ReusableBtn`/`ListRow`/`ChannelCard`/`StationRow`/`ProgramRow`/`SegmentedToggle` (`onFocus` + theme `focus` token); `hasTVPreferredFocus` per screen; `TVFocusGuideView` to bridge header↔grid; remote back/menu via `useTVEventHandler` + `BackHandler` → existing nav; player D-pad (reveal overlay, play/pause/seek); focus-trap the parental-PIN modal + the **ad `AdOverlay`** (skip/countdown focusable); make the bottom tab bar focusable.
    - **Phase D — 10-foot tuning:** tune `GRID_COLUMNS.tv` / `UI_SCALE.tv` / `CONTENT_MAX_WIDTH` on screen; disable mobile-only affordances on TV (pull-to-refresh, swipe).
    - **Phase E — STB self-update (deferred):** the `getAppVersion` poll → download/install-APK flow for sideloaded STBs (no Play Store) — see 11.X.11 carry-over.
    - **Risks to validate early:** expo-video AES-128 on TV (player-key-header risk, flagged in PLAYER.md; stream UA headers via `getStreamHeaders` must forward on TV too); `@expo/ui` Picker (`SelectInput`, education field) D-pad focus on Android Compose; bottom-tab pattern acceptability on a 10-foot UI; STOMP `ws://` over the TV network path (cleartext is dev/preview-gated per `c409250` — prod uses `wss://`).
