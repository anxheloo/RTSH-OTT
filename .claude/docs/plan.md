...
- [~] **22.18** Tablet / iPad / **TV** large-screen pass (decisions 8 + TV scope). *Foundation landed 2026-06-18:* the portable **`@/responsive`** module (device-class + orientation grid columns via shortest-side classifier, `scaled()` per-class token step) is in place and wired to the Home grid + `FONTSIZE`/`SPACING`/control tokens. **Tablet content-width pass landed 2026-06-25:** `useContentWidth(variant)` (`CONTENT_MAX_WIDTH` form 480 / content 640 / player 820 — centered cap on tablet/TV, no-op on phone) wired across auth forms, change-password, settings, account, profile, radio, the channel player (video + day-strip + EPG, conditional style preserving the single-tree rotation invariant), and Guide / Search row lists. **Remaining for this step:** on-device tuning of `GRID_COLUMNS`/`UI_SCALE`/`CONTENT_MAX_WIDTH`, and the **TV** sub-pass below. ~~Mosaic density (4/6/9)~~ — moot, mosaic cut 2026-06-11 (22.14f).
  - **22.18-TV — Android TV + STB enablement (decided 2026-06-25; verified vs SDK 56 TV guide).** Scope decisions: test via local prebuild + Android TV emulator (and EAS `*_tv` + Orbit); **full-polish** focus/D-pad nav across ALL screens; **keep the bottom Native Tabs, made focusable** (no separate TV nav layout); **STB self-update deferred** to a later sub-step.
    - **Phase A — build & boot:** add `@react-native-tvos/config-tv` (dev dep) to `app.config.ts` plugins (gate on `EXPO_TV`); swap `package.json` core to `"react-native": "npm:react-native-tvos@0.85-stable"` (drop-in superset — mobile builds unaffected, TV-ness is build-time via `EXPO_TV=1`); add EAS profiles `development_tv` / `preview_tv` (`env: { EXPO_TV: "1" }`) + `preview_stb` (`EXPO_TV:1` + `APP_PLATFORM: androidstb`); `EXPO_TV=1 npx expo prebuild --clean` → `expo run:android` on a TV emulator. **Local native dirs are gitignored CNG output** — prebuild regenerates them; toggling `EXPO_TV` swaps the local project between mobile/TV. Verify `Platform.isTV` → `getDeviceClass()='tv'` (4/4 grid, 1.3× scale, content caps) + backend `deviceClass` `TV`/`STB`.
    ...
    - **Risks to validate early:** expo-video AES-128 on TV (player-key-header risk, flagged in PLAYER.md); `@expo/ui` Picker (`SelectInput`) D-pad focus on Android Compose; bottom-tab pattern acceptability on a 10-foot UI.
