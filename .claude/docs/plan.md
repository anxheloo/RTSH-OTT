...
8. **Mobile-first, responsive later.** [HIGH] Grids 2-col on phones → `flexWrap` rows on tablet via `useWindowDimensions` (22.18). Applies to the channel grid (22.7); ~~mosaic (22.12)~~ cut 2026-06-11.
...
- [~] **11.X.9** Final endpoint wiring — **auth slice DONE 2026-06-12 (see 11.X.12)**; remaining domains (channels/EPG/catchup/radio/streams→playback, search, packages, guide) still deferred until their shapes are reviewed against the swagger.
  - **Carry-overs:** `useChannelsQuery` → `useInfiniteQuery`; EPG by channel+date (`useEpgQuery(channelId, date)`); register birthday field → native date picker; parental endpoints moved to `/parental` + `/parental/verify-pin` on the backend (ours still `/users/parental-pin*` — reconcile with the parental contract).
...
- [x] **22.7** Home (Kreu) — `BrandHeader` + mosaic/profile buttons, `SearchBar`, TV/Radio `SegmentedToggle`, `HeroCarousel`, `ContinueRow`, package `FilterChipRow`, 2-col `ChannelCard` grid; new `SceneBackground`/`SectionHeader`; `useHomeFeedQuery`. *(Superseded in part by 22.14f: mosaic button, `ContinueRow` rail and the "Guida" section link removed.)*
- [x] **22.8** Guide — `TabHeader` + TV/Radio toggle, `GuideRow` now/next + elapsed-progress from EPG; radio rows = name+genre+LIVE. *(Superseded in part by 22.14f: header → shared `BrandHeader`, toggle moved into the body.)*
...
- [x] **22.14e** Adaptive video quality (Auto + manual). Brainstorm 2026-06-11 (`docs/superpowers/specs/2026-06-11-adaptive-quality-design.md`). Wires the quality picker (22.10/22.13 left it inert) into the real player + closes 22.13 gap 1.
  ...
  - **Trade-offs / known gaps / VERIFY LATER:**
    ...
    3. **Backend contract unconfirmed** — does `/streams` return `masterUrl` / `renditions` / only `hlsUrl`? Add real per-rendition URLs in the mock (commented template in `handlers.ts`) + update `docs/API.md` at **11.X.9**.
...
- [~] **22.18** Tablet / iPad / **TV** large-screen pass (decisions 8 + TV scope). **Deferred until mobile (22.1–22.17) is complete + approved.** Same design, display adjustments only: `useWindowDimensions` breakpoints flip the channel grid (22.7) 2-col → `flexWrap` rows; revisit gutters/hero/player width. ~~Mosaic density (4/6/9)~~ — moot, mosaic cut 2026-06-11 (22.14f). **TV in v1 scope** (end-phase): focus/D-pad nav + 10-foot spacing on top of the large-screen layout (its own sub-pass after tablet). Build mobile first, then widen.
